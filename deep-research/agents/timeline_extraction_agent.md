...
This is the load-bearing component of v3.9.4 temporal verification. `bibliography_agent` is read-only context (you read its annotated bibliography); the boundary defined by phase-boundary spec is unchanged.
...
- `phase2_investigation/version_records.yaml` (academic citation version-family evidence for preprint -> proceedings -> journal chains)
...
3. Compute `confidence` per the agreement table (covering all source-state × outcome combinations including Crossref outage).
4. Write the entry to `phase2_investigation/citation_provenance.yaml`.

The downstream verifier looks up `confidence` for each `<!--ref:slug-->` marker; `low` or `conflict` causes the verifier to emit `TEMPORAL-METADATA-MISSING` rather than use the date as arithmetic ground truth.
...
3. Determine `supersedes` / `superseded_by` if the user has tagged related editions with shared `version_family_id` (user-declared only in v3.9.4 stub; future versions add academic citation version-family candidate discovery).
...
## Date precision discipline

Every date in `timeline.yaml` MUST carry `precision` ∈ {day, month, year, interval, unknown} and `provenance.method`. When `precision: unknown` AND `open_ended: false` (or absent), the verifier treats it as missing data. When `precision: unknown` AND `open_ended: true`, the verifier treats it as `+∞` (still in force; only valid for `effective_date_range.end`). No other date may carry `open_ended: true`.

## Academic Citation Version Discovery

For academic citation chains, write `phase2_investigation/version_records.yaml`. This extends the version_family_id stub from institutional documents to scholarly works that appear as arXiv preprints, conference proceedings, journal extensions, reports, datasets, or book chapters.
